...
class SuperStyler(object):
    ...
    def show_dialog(self):
        ...
        self.frm.webView.setHtml(get_body())
        self.frm.webView.setStyleSheet(get_stylesheet())
    ...
    def handle_open(self, link):
        ...
        server = get_open_server(model, tmpl)
...
def get_tmpl_row(model, tmpl, color):
    ...
    editor_column = "<td/>"
    if has_server and get_ss_dyndeck(model) is not None:
...
select c.id from cards c, notes f where c.nid=f.id and mid = ? and ord = ?""",
    ...
    dyndeck = get_ss_dyndeck(model)
    if dyndeck is not None:
        mw.col.decks.rem(dyndeck['id'])
    
def handle_create(link):
    m = re.search("create note:'(.+)' tmpl:'(.+)'", link)
    model_id = m.group(1)
    tmpl_ord = int(m.group(2))
    model = mw.col.models.get(model_id)
    tmpl = model['tmpls'][tmpl_ord]
    start_template_server(model, tmpl)
